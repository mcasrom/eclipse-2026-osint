...
- **[HECHO 06-Ago] NearMe Sprint 41 — Timeline/playback + track patterns UI**: backend (tabla , , , retención 365) + frontend (panel playback 7 días + **patrón por evento** con tira de estados y cambios de nivel, commit ) (tabla `event_history` con snapshots al cambiar, `GET /api/timeline` activos por ventana, `GET /api/event/{id}/history` track patterns, `cleanup_history_retention(365)` + cron diario 03:00) + frontend (panel con gráfico 7 días, slider de tiempo que filtra markers por `created_at/expires_at`, reproducción) — commits `c4cfef3` + `b41291c`. **Estimación disco 1 año de historial: ~0,8-1,8 GB** (Postgres), estable si se mantiene la rotación de 365 días.
