...
- **[REVISAR 1º] Funcionalidades rotas de MyIP** (06-Ago, tras rebase/docker rebuild). **Plan mañana**:
  1. **Reconstruir el contenedor MyIP** desde el código actual (rebase `d1656c0` + WIP local): `cd /home/deploy/myip && npm run build && sudo docker compose build myip && sudo docker compose up -d myip` (el contenedor quedó con un dist intermedio, sin todo el WIP).
  2. **Testear cada feature** en myip.viajeinteligencia.com: auth (registro/login/guest), escaneo, dashboard/historial, heatmap (ThreatMap), snapshots timeline, alertas email, stats comunidad, Ko-fi. Buscar errores de consola (el WIP toca AuthSection/App/Server).
  3. **Revisar la landing** (viajeinteligencia-landing): el mapa embebido de emergency usaba `emergencies_history.json` → ahora obsoleto/roto (emergency eliminado). Decidir quitar el bloque o dejarlo estático.
  4. Si algo no compila: **backups** MyIP → `/home/deploy/backups/myip-20260805/myip-full.tgz`; WIP untracked → `/tmp/myip-untracked/`. Git sincronizado (`d1656c0`).
